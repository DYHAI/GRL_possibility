#!/usr/bin/env julia
"""Export multi-member ensemble to NPZ; default saves all frames per member (KH_EXPORT_ALIGN=1 for intersection)."""
...
const FIELD_VARS = ("rho", "v1", "v2", "p")

"""Fixed-width UInt8 matrix; NPZ.jl cannot write Vector{String}."""
...
function read_ensemble_meta(out_dir::String)
    ...
    if save_dt === nothing && haskey(ENV, "KH_SAVE_DT") && !isempty(strip(ENV["KH_SAVE_DT"]))
        save_dt = parse(Float64, ENV["KH_SAVE_DT"])
    end
...
function h5_time(h5_dir::String, step::Int)
    path = joinpath(h5_dir, @sprintf("solution_%09d.h5", step))
    HDF5.h5open(path, "r") do file
        Float64(read(attributes(file)["time"]))
    end
end
...
function time_index(t::Float64, t0::Float64, save_dt::Float64)
    round(Int, round((t - t0) / save_dt))
end

function export_aligned!(dict, members, member_frames, save_dt::Union{Nothing,Float64})
    ...
    if save_dt !== nothing
        ...
        for (fi, ti) in enumerate(common_ti)
            ...
            for mid in members
                ...
                dict["cx_$(mid)_$(fi-1)"] = fr["cx"]
                dict["cy_$(mid)_$(fi-1)"] = fr["cy"]
            end
            ref = buckets[ti][members[1]]
            dict["step_$(fi-1)"] = Int32(ref["step"])
            dict["time_$(fi-1)"] = Float32(ref["time"])
            println("  aligned frame $(fi-1)  ti=$ti  t=$(round(ref["time"], digits=4))")
        end
        dict["times"] = times
        ...
        for mid in members
            ...
            for fi in 0:(all_n_frames-1)
                ...
                dict["cx_$(mid)_$(fi)"] = fr["cx"]
                dict["cy_$(mid)_$(fi)"] = fr["cy"]
                if mid == members[1]
...
"""Save every frame each member has; keys rho_{m}_{fi} with fi in 0..nframes_m-1."""
function export_per_member!(dict, members, member_frames)
    dict["align_frames"] = Int32(0)
    n_max = maximum(length(member_frames[mid]) for mid in members)
    dict["n_frames_max"] = Int32(n_max)
    dict["n_frames"] = Int32(n_max)  # upper bound; check member_{m}_nframes per member
    for mid in members
        frames = member_frames[mid]
        nf = length(frames)
        dict["member_$(mid)_nframes"] = Int32(nf)
        times_m = Float32[Float32(fr["time"]) for fr in frames]
        dict["member_$(mid)_times"] = times_m
        for (fi, fr) in enumerate(frames)
            fi0 = fi - 1
            for var in FIELD_VARS
                dict["$(var)_$(mid)_$(fi0)"] = fr[var]
            end
            dict["cx_$(mid)_$(fi0)"] = fr["cx"]
            dict["cy_$(mid)_$(fi0)"] = fr["cy"]
            dict["step_$(mid)_$(fi0)"] = Int32(fr["step"])
            dict["time_$(mid)_$(fi0)"] = Float32(fr["time"])
        end
        t_first = isempty(frames) ? 0.0 : frames[1]["time"]
        t_last = isempty(frames) ? 0.0 : frames[end]["time"]
        println("  member $mid: $nf frames  t=[$t_first, $t_last]")
    end
end
...
dict["var_names"] = npz_encode_strings(collect(FIELD_VARS))
dict["n_channels"] = Int32(length(FIELD_VARS))
...
if align_frames
    export_aligned!(dict, members, member_frames, save_dt)
else
    export_per_member!(dict, members, member_frames)
end

npz_out = joinpath(out_dir, "ensemble_snapshots.npz")
npzwrite(npz_out, dict)
mode = align_frames ? "aligned" : "per_member"
println(">>> Wrote $npz_out  mode=$mode  members=$(members)" *
        "  n_frames=$(dict["n_frames"])" *
        "  channels=$(join(FIELD_VARS, ","))" *
